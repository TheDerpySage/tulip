...
def is_super(ctx):
	return (ctx.message.author.id == bot_config.owner_id) or (ctx.message.author == ctx.message.guild.owner)

#def enforce_mod_only(self, ctx):
#	if self.mod_only == True:
#		return (ctx.message.author.id == in ur list of ids or something idk)
#	else: return True
...
class AdminCog(commands.Cog):
	...
	async def credits(self, ctx):
 	...
 	#@commands.command(aliases=["God Mode"])
	#async def mod_only(self, ctx):
	#	if self.mod_only == True:
	#		self.mod_only == False
	#		ctx.send("God Mode Disbaled")
	#	else:
	#		self.mod_only == True
